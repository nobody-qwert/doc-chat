...
- **Answer composition** – `compose_answer()` is run in streaming or buffered mode depending on the endpoint. It receives only the curated evidence and must cite doc hashes directly. The streaming variant yields incremental tokens plus metadata about the prompts that were used.
